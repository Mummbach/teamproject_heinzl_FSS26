...
# Outlier clipping for aggregated vital features
# True  — clip aggregated vital columns (slope, std, mean, min, max, …) to
#         [WINSORIZE_LOW, WINSORIZE_HIGH] percentile range fitted on train set.
...
# WINSORIZE aggregated vital features (fitted on train only)
if WINSORIZE_VITALS and USE_AGGREGATED_VITALS:
    print(f"\nWinsorizing aggregated vital features "
          f"(p{WINSORIZE_LOW*100:.0f}–p{WINSORIZE_HIGH*100:.0f}, fitted on train)...")

    vital_agg_cols = [
        c for c in ts_flat.columns
        if c != "stay_id" and not c.endswith("_missing")
    ]
    vital_agg_cols = [c for c in vital_agg_cols if c in X_train.columns]

    lower_bounds = X_train[vital_agg_cols].quantile(WINSORIZE_LOW)
    upper_bounds = X_train[vital_agg_cols].quantile(WINSORIZE_HIGH)

    n_clipped_total = 0
    for name, X_split in [("train", X_train), ("val", X_val), ("test", X_test)]:
        n_before = ((X_split[vital_agg_cols] < lower_bounds) |
                    (X_split[vital_agg_cols] > upper_bounds)).sum().sum()
        X_split[vital_agg_cols] = X_split[vital_agg_cols].clip(
            lower=lower_bounds, upper=upper_bounds, axis=1
        )
        n_clipped_total += n_before
        print(f"  {name:<6}: {n_before:>6,} values clipped")

    print(f"  Total values clipped: {n_clipped_total:,}")

    bounds_df = pd.DataFrame({
        "feature": vital_agg_cols,
        "lower":   lower_bounds.values,
        "upper":   upper_bounds.values,
    })

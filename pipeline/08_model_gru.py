...
Flags:
  USE_TEXT  — True:  include CXR text branch (requires 03_cxr_features.py output)
              False: disable text branch (baseline, no CXR data needed)
  CXR_ONLY  — True:  restrict cohort to patients with a CXR report (~17.5%)
              False: full cohort — patients without report get zero vector

Run AFTER:  07_normalize.py
            03_cxr_features.py  (if USE_TEXT = True)
...
Input:   output/X_train_scaled.parquet  /  X_val_scaled  /  X_test_scaled
         ...
         output/cxr_bert_embeddings.parquet  (only if USE_TEXT = True)
...
# ── Flags ─────────────────────────────────────────────────────────────
USE_TEXT = True    # include BioClinicalBERT CXR text branch
CXR_ONLY = False   # restrict to patients with CXR report
...
DROPOUT       = 0.3
STATIC_DIM    = 64
TEXT_DIM      = 32   # projection size for 1536-dim CXR embeddings
...
class ICUDataset(Dataset):
    ...
    def __init__(self, X_static: pd.DataFrame, y: pd.DataFrame,
        ...
        # CXR embeddings — zero vector for patients without report
...
class GRUModel(nn.Module):
    ...
    Args:
        ...
        text_dim         : projection size for 1536-dim CXR embeddings
    """
...
def train_epoch(model, loader, optimizer, criterion):
    ...
    for ts, static, text, labels in loader:
        ...
        optimizer.zero_grad()
        logits = model(ts, static, text if USE_TEXT else None)
...
# ── Load CXR embeddings (optional) ────────────────────────────────────
cxr = None
if USE_TEXT:
    cxr_path = OUTPUT_DIR / "cxr_bert_embeddings.parquet"
    if not cxr_path.exists():
        raise FileNotFoundError(
            "cxr_bert_embeddings.parquet not found. Run 03_cxr_features.py first."
        )
    cxr = pd.read_parquet(cxr_path)
    print(f"  CXR embeddings loaded: {len(cxr):,} stays")

    if CXR_ONLY:
        cxr_stay_ids = set(cxr["stay_id"])
        X_train = X_train[X_train["stay_id"].isin(cxr_stay_ids)]
        X_val   = X_val[X_val["stay_id"].isin(cxr_stay_ids)]
        X_test  = X_test[X_test["stay_id"].isin(cxr_stay_ids)]
        y_train = y_train[y_train["stay_id"].isin(cxr_stay_ids)]
        y_val   = y_val[y_val["stay_id"].isin(cxr_stay_ids)]
        y_test  = y_test[y_test["stay_id"].isin(cxr_stay_ids)]
        print(f"  CXR_ONLY=True: cohort restricted to {len(X_train):,} train stays")
...
train_ds = ICUDataset(X_train, y_train, ts, TS_FEATURES, cxr=cxr)
val_ds   = ICUDataset(X_val,   y_val,   ts, TS_FEATURES, cxr=cxr)
test_ds  = ICUDataset(X_test,  y_test,  ts, TS_FEATURES, cxr=cxr)
...
def get_predictions(model, loader, stay_ids):
    model.eval()
    all_logits = []
    for ts, static, text, _ in loader:
        ts, static, text = ts.to(DEVICE), static.to(DEVICE), text.to(DEVICE)
        all_logits.append(model(ts, static, text if USE_TEXT else None).cpu().numpy())

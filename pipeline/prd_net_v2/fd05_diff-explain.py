...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        probs = torch.sigmoid(model(torch.from_numpy(Xte).float())).numpy()
    preds = (probs >= thr).astype(int)

    # ── GLOBAL importance, split by side ──────────────────────────────────────
    mean_abs = np.abs(shap_vals).mean(axis=0)            # (in_dim,) — reused in plots below
    ...
    def render(i, shap_vals, Xte, labels_in, feats, abs_feats, scale, mean_, F, top=5):
    ...
    if true_long.size == 0:
        print("\nSanity — no true long-stay patients in test set; skipping block.")
    else:
        conf = true_long[np.argsort(probs[true_long])[::-1][:5]]
        print(f"\nSanity — top-{len(conf)} most-confident TRUE long-stay patients:")
        n_consistent = 0
        for i in conf:
            net = shap_vals[i].sum()
            ok = bool(preds[i])                          # logit(i) > 0 = model predicts long-stay
            n_consistent += ok
            ...
            for line in render(i, shap_vals, Xte, labels_in, feats, abs_feats, scale, mean_, F):
                print(line)
    ...
    # base_value = explainer.expected_value (logit space).
    # Invariant: base_value + shap_vals[i].sum() == logit(i) for every patient i.
    ...
    base_value      = float(explainer.expected_value)   # logit space
    TOP_N_WATERFALL = 15
    ...
    for case_name, i in patient_cases.items():
        ...
        ax.axvline(base_value, color="black", linewidth=1.2, linestyle="--",
                   label=f"E[f(x)] = {base_value:.3f} (logit)")
        ax.axvline(y_logit, color="gray", linewidth=1.2, linestyle=":",
                   label=f"f(x) logit = {y_logit:.3f}  (prob={y_prob:.3f})")
    ...
    top3_j     = np.argsort(mean_abs)[::-1][:3]       # reuses mean_abs from global section
    ...
    for ax, j, name in zip(axes, top3_j, top3_names):
        ...
        ax.set_title(f"{name}\nmean|SHAP| = {mean_abs[j]:.4f}", fontsize=9)
    ...
    ax1.plot(mean_pred, frac_pos, "o-", color="#d73027",
             linewidth=2, markersize=6, label="Model")
    ax1.fill_between(mean_pred, frac_pos, mean_pred,
